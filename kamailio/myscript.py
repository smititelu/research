...
class ModuleClass:
	def my_child_init(self, arg):
		print 'Hello, child!'
		return 1

	def my_python_function(self, msg, args):
...
#		Router.Logger.LM_INFO(str(posX_first) + " " + str(posY_first) + " " + str(posX_last) + " " + str(posY_last) + "\n")
		...
		Router.Logger.LM_INFO("euclidian distance=" + str(euclid) + "\n")

		if (euclid > hysteresis):
			Router.Logger.LM_INFO("euclidian distance=" + str(euclid) + " outside perimeter=" + str(hysteresis) + " HANDOVER !!!\n")
